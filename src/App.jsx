...
import { addItem, addBasketCount, addBasketSum } from './redux/slices/cartSlice';
	...
	async function fetchMainData(page) {
		try {
			const bitrixResponse = await axios.get(
				`/bitrix/services/main/ajax.php?c=goodde:ajax&mode=class&action=elementlist&page=${page}`,
			);
			...
			bitrixResponse.data.list.map((item) => {
				const itemHomeData = {
					id: Number(item.ID),
					defaultPicture:
						item.RESIZE_IMAGE && isWebpUse.current == true
							? item.RESIZE_IMAGE.PIC_WEBP
							: item.RESIZE_IMAGE && isWebpUse.current == false
		...
		async function fetchData() {
			try {
				...
				const [categoryResponse, hitResponse, cartResponse] = await Promise.all([
					...
					axios.get(
						`/bitrix/services/main/ajax.php?c=goodde:ajax&mode=class&action=elementlisthit`,
					),
